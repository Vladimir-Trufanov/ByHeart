 ...
 * transmission.
 ...
 с переданными вручную (неавтоматическими) пакетами подтверждения (ACK). 
 В этом примере по-прежнему используются пакеты подтверждения, но они не 
 содержат полезной нагрузки. Вместо этого подтверждающий ответ отправляется 
 с помощью "write()". Эта тактика позволяет получать более обновленные 
 данные полезной нагрузки подтверждения, в то время как фактические данные 
 полезной нагрузки подтверждения устаревают на 1 передачу, поскольку их 
 необходимо загрузить перед получением передачи.
 *
