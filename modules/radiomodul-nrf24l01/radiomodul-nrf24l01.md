...
### [Справочник по радио-модулям NRF24L01+ / PA+LNA 2.4G](https://wiki.iarduino.ru/page/NRF24L01-trema/)
...
### [Oбзор модуля nRF24L01](#%D0%BE%D0%B1%D0%B7%D0%BE%D1%80-%D0%BC%D0%BE%D0%B4%D1%83%D0%BB%D1%8F-nrf24l01)
...
### [Конденсатор, антенна для стабильной работы](#%D0%BA%D0%BE%D0%BD%D0%B4%D0%B5%D0%BD%D1%81%D0%B0%D1%82%D0%BE%D1%80)

---

### [Обзор модуля nRF24L01](https://3d-diy.ru/blog/radio-modul-nrf24l01/)
...
Ещё одним вариантом решения проблемы запуска, является использование дополнительного адаптера со встроенным стабилизатором напряжения. В таком случае для NRF24L01 можно использовать ***внешнее питание от 4.8В до 12В***, а максимальный выходной ток составит 800мА. 

![](adapter_nrf24l01.webp)
...
Мощность передатчика модуля NRF24L01 можно программно задать с помощью функции ***setPALevel()***. Эта функция устанавливает уровень усиления мощности (PA — Power Amplifier). 

Функция setPALevel() принимает следующие константы, соответствующие разным уровням мощности в дБм: RF24_PA_MIN — −18 дБм; RF24_PA_LOW — −12 дБм; RF24_PA_HIGH — −6 дБм; RF24_PA_MAX — 0 дБм (максимальная мощность). 

```
radio.setPALevel(RF24_PA_MAX);  // Устанавливаем максимальную мощность передатчика
```
При настройке передатчика можно задать канал передачи данных (от 0 до 127) с помощью функции ***setChannel()***  и скорость передачи данных с помощью функции ***setDataRate()*** (значения: RF24_250KBPS, RF24_1MBPS, RF24_2MBPS). 

> ***Важно, чтобы на стороне приёмника были заданы те же параметры, включая мощность передатчика.***

Примечание: при работе с модулем NRF24L01 необходимо учитывать ограничения по использованию каналов и другие технические характеристики. Например, не допускается использование каналов 84–99 [0x54-0x63] (2484–2499 МГц) без внешних усилителей. 
...
![](Nrf24l01-and-arduino-5.jpg)

4. [100 мкФ](https://3d-diy.ru/blog/radio-modul-nrf24l01/)

4.1. При передачи сигнала радио модуль NRF24L01 кратковременно может потреблять большой ток, поэтому рекомендуется по питанию установить электролитический конденсатор [емкость от 10 до 220 мкФ](http://rcl-radio.ru/?p=128866).
...
Технология здесь следующая: чтобы у нас ничего не замкнуло, мы сначала обматываем плату простой бумагой, после чего укрепляем эту бумагу одним-двумя витками скотча, а уже поверх скотча наматываем фольгу и поверх фольги заново укрепляем её скотчем. ***Причём нужно обмотать фольгой как плату передатчика, так и приёмника.***
...
#### [nRF24L01+ однокристальный трансивер 2.4 ГГц](https://microsin.net/adminstuff/hardware/nrf24l01-single-chip-24ghz-transceiver.html)
